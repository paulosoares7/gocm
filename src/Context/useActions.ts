...
export function useActions(){
  ...
  useEffect(()=>{
    ...
    const localStorageParticule = JSON.parse(localStorageParticules) !== null ? JSON.parse(localStorageParticules) : []
    setParticules(localStorageParticule)

    let localStorageInputTime = localStorage.getItem('timeSimulation') as string
    const localStorageTime = JSON.parse(localStorageInputTime) !== null ? JSON.parse(localStorageInputTime) : [0]
  ...
  async function CreateNewParticule(particulesInput: ParticuleInputProps ){
    ...
    const particule = {...particulesInput,background: randomColor(),id: idGenerate}
