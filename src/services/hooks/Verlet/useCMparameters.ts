...
export function useCMparameters() {
  ...
  const paramentCM = particules.map((particule: Particule) => ({
    ...
    fx:
      Math.cos((Number(particule.degrees) * Math.PI) / 180) *
      Number(particule.force),
    fy:
      Math.sin((Number(particule.degrees) * Math.PI) / 180) *
      Number(particule.force),
  ...
  const mCm = paramentCM.reduce(
    (preVal: number, particule) => Number((preVal + particule.mass).toFixed(4)),
    0
  );
  // somatório: x[i].m[i]
  const xCm = paramentCM.reduce(
    (preVal: number, particule) =>
      Number(
        (preVal + (particule.positionX * particule.mass) / mCm).toFixed(4)
      ),
    0
  );
  ...
  const yCm = paramentCM.reduce(
    (preVal: number, particule) =>
      Number(
        (preVal + (particule.positionY * particule.mass) / mCm).toFixed(4)
      ),
    0
  );
  ...
  const aCmX = paramentCM.reduce(
    (preVal: number, particule) =>
      Number((preVal + particule.fx / mCm).toFixed(4)),
    0
  );
  ...
  const aCmY = paramentCM.reduce(
    (preVal: number, particule) =>
      Number((preVal + particule.fy / mCm).toFixed(4)),
    0
  );

  const fx = paramentCM.reduce(
    (preVal: number, particule) => Number((preVal + particule.fx).toFixed(4)),
    0
  );

  const fy = paramentCM.reduce(
    (preVal: number, particule) => Number((preVal + particule.fy).toFixed(4)),
    0
  );

  const fr = paramentCM.reduce(
    (preVal: number, particule) =>
      preVal + Math.sqrt(Math.pow(particule.fx, 2) + Math.pow(particule.fy, 2)),
    0
  );
